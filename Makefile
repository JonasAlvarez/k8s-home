...
.PHONY: uninstall
uninstall: switch-kubectl-context uninstall-all
...
.PHONY: uninstall-all
uninstall-all: $(foreach app,$(APPS_UNINSTALL),uninstall-$(app)) ## Uninstall all applications

.PHONY: uninstall-%
uninstall:
uninstall-%: switch-kubectl-context ## Uninstall a single application
	$(MAKE) -C $* -e ENVIRONMENT=$(ENVIRONMENT) uninstall
